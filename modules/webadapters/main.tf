locals {
  ...
  lambda_map = {
    for k, v in var.lambda : k => {
      ...
      "policy_statements"                 = coalesce(lookup(v, "policy_statements", null), local.merged_default_settings.policy_statements)
...
data "aws_lambda_function" "lambda_function" {
  for_each      = { for k, v in local.lambda_map : k => v if v.image_uri == "" }
...
module "webadapter" {
  ...
  version                           = "~> 7.2.1"
  function_name                     = each.value.identifier
  description                       = "Lambda ${each.value.identifier} webadapter"
  ...
  vpc_subnet_ids                    = each.value.vpc_id == "" ? null : each.value.subnet_ids
  vpc_security_group_ids            = each.value.vpc_id == "" ? [] : [module.lambda_sg[each.key].security_group_id]
  attach_network_policy             = each.value.vpc_id == "" ? false : true
  ...
  environment_variables = merge(
    ...
    {
      LAMBDA_CONFIG_PROJECT_NAME = each.value.identifier
      LAMBDA_CONFIG_AWS_REGION   = data.aws_region.current.name
    },
    lookup(local.secret_vars_env, each.key, {})
  )
  package_type = "Image"
  image_uri    = each.value.image_uri == "" ? data.aws_lambda_function.lambda_function[each.key].image_uri : each.value.image_uri
  tags         = local.tags
}

module "lambda_sg" {
  for_each    = { for k, v in local.lambda_map : k => v if v.vpc_id != "" }
  source      = "terraform-aws-modules/security-group/aws"
  version     = "~> 5.1.0"
  create      = each.value.vpc_id != ""
...
resource "aws_cloudwatch_event_target" "cron" {
  ...
  arn       = module.webadapter[each.key].lambda_function_arn
}
...
resource "aws_lambda_permission" "cloudwatch" {
  ...
  function_name = module.webadapter[each.key].lambda_function_name
...
resource "aws_lambda_permission" "lambda_permission" {
  ...
  action              = "lambda:InvokeFunction"
  function_name       = module.webadapter[split("|", each.key)[0]].lambda_function_name
...
resource "aws_cloudwatch_event_target" "lambda" {
  for_each  = local.eventbridge_rule_map
  target_id = "lambda"
  arn       = module.webadapter[split("|", each.key)[0]].lambda_function_arn
