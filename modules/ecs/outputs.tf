...
output "ecs_scheduled_tasks" {
  value = {
    for k, v in aws_scheduler_schedule.ecs_scheduled_task : k => {
      ...
      task_definition_arn          = local.scheduled_task_ecs_service_resources[k].task_definition_arn
      task_definition_family       = local.scheduled_task_ecs_service_resources[k].task_definition_family
      task_exec_iam_role_name      = local.scheduled_task_ecs_service_resources[k].task_exec_iam_role_name
      task_exec_iam_role_arn       = local.scheduled_task_ecs_service_resources[k].task_exec_iam_role_arn
      task_exec_secret_arns        = try(local.scheduled_task_exec_secret_arns[local.scheduled_task_ecs_service_key[k]], [])
      task_runtime_iam_role_name   = local.scheduled_task_ecs_service_resources[k].tasks_iam_role_name
      task_runtime_iam_role_arn    = local.scheduled_task_ecs_service_resources[k].tasks_iam_role_arn
