...
output "ecs_scheduled_tasks" {
  value = {
    for k, v in aws_cloudwatch_event_rule.ecs_scheduled_task : k => {
      ...
      task_exec_iam_role    = data.aws_ecs_task_definition.scheduled_task_reuse[k].execution_role_arn
      task_runtime_iam_role = data.aws_ecs_task_definition.scheduled_task_reuse[k].task_role_arn
    }
  }
}
