...
locals {
  ...
  apigateway_map = {
    for k, v in var.apigateway : k => {
      ...
      "route_key"                   = coalesce(lookup(v, "route_key", null), local.merged_default_settings.route_key)
...
locals {
  apigateway_routekey_map = merge([
    for k, v in local.apigateway_map : {
      for route_key in keys(v.route_key) : "${k}|${route_key}" => v.route_key[route_key]
    } if length(v.route_key) > 0 && v.create
  ]...)
}
...
resource "aws_lambda_permission" "api_gateway" {
  ...
  source_arn    = "${aws_apigatewayv2_api.apigateway[split("|", each.key)[0]].execution_arn}/*/${split("|", each.key)[1]}"
}
