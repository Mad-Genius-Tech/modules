...
locals {
  overall_budget_map = {
    for k, v in var.overall_budget : k => {
      ...
      include_discount           = coalesce(v.include_discount, true)
      include_other_subscription = coalesce(v.include_other_subscription, true)
      include_recurring          = coalesce(v.include_recurring, true)
      include_refund             = coalesce(v.include_refund, true)
      include_subscription       = coalesce(v.include_subscription, true)
      include_support            = coalesce(v.include_support, true)
      include_tax                = coalesce(v.include_tax, true)
      include_upfront            = coalesce(v.include_upfront, true)
      use_amortized              = coalesce(v.use_amortized, false)
      use_blended                = coalesce(v.use_blended, false)
    }
  }
